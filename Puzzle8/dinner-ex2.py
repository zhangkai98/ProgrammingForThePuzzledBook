...
def InviteDinnerSimplified(guestList, dislikePairs):
    guestListConcernd = []
    for i in dislikePairs:
        if not i[0] in guestListConcernd:
            guestListConcernd.append(i[0])
        if not i[1] in guestListConcernd:
            guestListConcernd.append(i[1])

    guestListNotConcernd = []
    for j in guestList:
        if not j in guestListConcernd:
            guestListNotConcernd.append(j)

    inviteFinal = InviteDinner(guestListConcernd, dislikePairs) + guestListNotConcernd
    print ('Optimum Solution:', inviteFinal)

dislikePairs = [['Alice','Bob'],['Bob','Eve']]
guestList = ['Alice','Bob','Cleo','Don','Eve']
InviteDinnerSimplified(guestList, dislikePairs)
...
InviteDinnerSimplified(LargeGuestList, LargeDislikes)
