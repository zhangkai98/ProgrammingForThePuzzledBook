...
class BSTree:
    ...
    #Add size() and __sizeHelper() method
    def size(self):
        outputSize = 0
        return self.__sizeHelper(self.root, outputSize)

    def __sizeHelper(self, vertex, outputSize):
        if vertex is None:
            return outputSize
        outputSize = self.__sizeHelper(vertex.getLeftChild(), outputSize)
        outputSize += 1
        outputSize = self.__sizeHelper(vertex.getRightChild(), outputSize)
        return outputSize
...
testVertex = tree.getVertex(22)
print ('testVertex value =', testVertex.getVal(),\
 'testVertex left child = ', testVertex.getLeftChild().getVal(), \
 'testVertex right child = ', testVertex.getRightChild().getVal())
