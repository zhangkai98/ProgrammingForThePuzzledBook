...
def optimalBST(keys, prob):
    ...
    memo = {}
    computeOptRecur(opt, 0, n-1, prob, memo)
    print('memo = ', memo)
...
def computeOptRecur(opt, left, right, prob, memo):

    #base case
    if left == right:
        if (left, left) not in memo:
            opt[left][left] = (prob[left], left)
            memo[(left, left)] = opt[left][left]
        return 
    ...
    for r in range(left, right + 1):
        if left <= r - 1:
            if (left, r-1) in memo:
                leftval = memo[(left, r-1)]
            else:
                computeOptRecur(opt, left, r - 1, prob, memo)
                leftval = opt[left][r-1]
        else:
            leftval = (0, -1)
        
        if r + 1 <= right:
            if (r+1, right) in memo:
                rightval = memo[(r+1, right)]
            else:
                computeOptRecur(opt, r + 1, right, prob, memo)
