...
##ADD my first rangeKeys function which was imitated from inOrder method.
##This function can work with low efficiency because it traversals all vertexs in the tree and do not 
##use the relationship of tree.
def rangeKeys(bst, k1, k2):
    outputKeysLists = []
    rangeKeysHelper(bst, k1, k2, bst.root, outputKeysLists)
    print(outputKeysLists)

def rangeKeysHelper(bst, k1, k2, vertex, outputKeysLists):
    if vertex == None:
        return 
    rangeKeysHelper(bst, k1, k2, vertex.getLeftChild(), outputKeysLists)
    if k1<=vertex.getVal() and k2>=vertex.getVal():
        outputKeysLists.append(vertex.getVal())
    rangeKeysHelper(bst, k1, k2, vertex.getRightChild(), outputKeysLists)

##ADD my second rangeKeys function with higher efficiency.
def rangeKeys2(bst, k1, k2):
    outputKeysLists = []
    rangeKeysHelper2(bst, k1, k2, bst.root, outputKeysLists)
    print(outputKeysLists)

def rangeKeysHelper2(bst, k1 ,k2, vertex, outputKeysLists):
    if vertex == None:
        return 

    if k2 <= vertex.getVal():
        rangeKeysHelper2(bst, k1, k2, vertex.getLeftChild(), outputKeysLists)
        if k2 == vertex.getVal():
            outputKeysLists.append(vertex.getVal())
    elif k1 >= vertex.getVal():
        if k1 == vertex.getVal():
            outputKeysLists.append(vertex.getVal())
        rangeKeysHelper2(bst, k1, k2, vertex.getRightChild(), outputKeysLists)
    else:
        rangeKeysHelper(bst, k1, k2, vertex.getLeftChild(), outputKeysLists)
        outputKeysLists.append(vertex.getVal())
        rangeKeysHelper(bst, k1, k2, vertex.getRightChild(), outputKeysLists)
...
rangeKeys(tree, 10, 22)
rangeKeys2(tree, 10 ,22)
'''
keys = [1, 2, 3, 4, 5, 6, 7]
