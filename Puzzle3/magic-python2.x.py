...
#Deck is represented as a list of strings, each string is a card
...
def AssistantOrdersCards():
    ...
    for i in range(5):
        #The , tells the print to not print a newline
        print 'Please give card', i+1,
        #In Python 2.x, we need to use raw_input() to get a string
        card = raw_input('in above format:')
        cards.append(card)
        n = deck.index(card)
        cind.append(n)
...
def ComputerAssistant():
    ...
    number = 0
    while number < 99999:
        number = int(raw_input('Please give random number of at least 6 digits:'))
    for i in range(5):
...
##    print cards
    ...
    guess = raw_input('What is the hidden card?')
