...
#This procedure checks if current solution satisfies the requirement.
def Required(cursolution, requiredlist):
    requiredindices = [i for i in range(len(requiredlist)) if requiredlist[i] >= 0]
    for c in requiredindices:
        if cursolution[c] != requiredlist[c]:
            return False
    return True

#This procedure places 8 Queens on a board so they don't conflict.
#It assumes n = 8 and won't work with other n!
def EightQueens(requiredlist, n=8):
    board = [-1] * n
    for i in range(n):
        board[0] = i
        for j in range(n):
                            ...
                            for p in range(n):
                                ...
                                for q in range(n):
                                    ...
                                    if noConflicts(board, 7):
                                        if Required(board, requiredlist):
                                            print(board)
